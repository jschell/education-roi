...
@ipeds_app.command("register-enrollment")
def ipeds_register_enrollment(
    catalog: Annotated[Path, typer.Option(exists=True, dir_okay=False, readable=True)],
    root: Annotated[Path | None, typer.Option(help="Project root.")] = None,
) -> None:
    """Validate and register paired final revised EF2023A sources."""
    try:
        release = select_release(
            IPEDSReleaseCatalog.from_file(catalog),
        ...
        registered = register_enrollment(release, ProjectPaths.from_environment(root))
    except (IPEDSCatalogError, IPEDSEnrollmentError, ProvenanceError, ValueError) as error:
        typer.echo(json.dumps({"error": str(error), "status": "INVALID"}, sort_keys=True))
        raise typer.Exit(code=2) from None
    typer.echo(
        json.dumps(
...
@ipeds_app.command("resolve-enrollment")
def ipeds_resolve_enrollment(
    ...
    """Return one final fall-2023 enrollment population, not a probability."""
    paths = ProjectPaths.from_environment(root)
    try:
        release = select_release(
            IPEDSReleaseCatalog.from_file(catalog),
